...
class EbdaAPI:
    ...
    def get_surveys_for_support_type(self, params: dict):
        ...
        try:
            ...
            frappe.log_error(title="Get Surveys For Support Type", message=frappe.get_traceback())
            frappe.throw(f"An error occurred: {e}")
            return None
...
def get_surveys():
    ...
    for params in support_type_list:
        if params.get("support_type_id") and params.get("date_from") and params.get("date_to"):
