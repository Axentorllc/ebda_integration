...
def build_params_for_support_types():
    support_types = frappe.get_all("Odoo Survey Support Type", fields=[
        "name", "odoo_id", "text", "last_synced_survey"
	])
    params = []
...
def get_answers_from(survays: list = []) -> list:
    ...
    for survey in survays:
        ...
        for answer in survey.get("answer_ids", []):
            ...
            if filtered_answer and not any(key.startswith("answer") for key in filtered_answer):
                filtered_answer.update({
...
def create_ebda_survey_from(answers: list, support_type_id: str):    
    ...
    for answer in answers:
        ...
        frappe.db.set_value("Odoo Survey Support Type", support_type_id, "last_synced_survey", now_datetime())

def create_ebda_survey(id, support_type_id, trouble_id, industrial_activity_id, gov_id, area_id,survey_answers):
